  ...
  const handleSearch = async () => {
    ...
    try {
      const response = await fetch(
        `http://10.6.24.109:5112/api/pokemon/${query.toLowerCase()}`,
      );

      if (!response.ok) {
        throw new Error("Pokémon nem található");
      }

      const data = await response.json();
      setPokemon(data.customPokemon);
      console.log("Pokémon adatok: ", pokemon.customPokemon);
    } catch (err) {
      setError("Nem található pokémon");
    } finally {
      setLoading(false);
    }
  };
